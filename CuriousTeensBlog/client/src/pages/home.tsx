...
export default function Home() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch('https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/@thecuriousteens/feed');
      if (!response.ok) {
  ...
  const BlogPostCard = ({ post, index }: { post: BlogPost; index: number }) => {
    ...
    return (
      <Card 
        className="overflow-hidden hover-lift fade-in bg-white border-border-light"
  ...
  const LoadingSkeleton = () => (
    <div className="space-y-8">
      {[1, 2, 3].map((i) => (
        <Card key={i} className="bg-white border-border-light p-8 hover-lift">
  ...
  return (
    <div className="bg-white text-text-primary font-sans min-h-screen">
      {/* Header */}
      <header className="border-b border-border-light bg-white sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-6 py-8">
          <div className="text-center">
            ...
            <a 
              href="https://medium.com/@thecuriousteens" 
              target="_blank" 
              rel="noopener noreferrer"
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white border border-border-light rounded-lg p-6 hover-lift">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white border border-border-light rounded-lg p-6 hover-lift">
              <h3 className="font-serif text-xl font-bold text-text-primary mb-4">
                About The Curious Teens
              </h3>
              <p className="text-text-secondary text-sm leading-relaxed mb-4">
                The Curious Teens is a young writers' team driven by imagination, curiosity, and fearless storytelling.
              </p>
              <p className="text-text-secondary text-sm leading-relaxed mb-4">
                We explore the weird, the wild, the scientific, and the supernatural — all through stories that make you think.
              </p>
              <p className="text-text-secondary text-sm leading-relaxed mb-4">
                Whether it's robots on the run or haunted buses, we write what others are afraid to imagine.
              </p>
              <div className="flex items-center text-sm text-text-primary font-medium">
                📚 Created by teens, for everyone who loves a twist.
              </div>
            </div>
            ...
            <div className="bg-white border border-border-light rounded-lg p-6 hover-lift">
              ...
              {rssData?.items?.slice(0, 3).map((post, index) => (
                <div key={`sidebar-${post.guid}`} className="mb-3 last:mb-0">
                  ...
                  {index < 2 && <hr className="my-3 border-border-light" />}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="border-t border-border-light bg-gray-50 mt-16">
        <div className="max-w-4xl mx-auto px-6 py-12 text-center">
          <p className="text-text-secondary mb-4">
            © 2025 The Curious Teens. All rights reserved.
          </p>
          <p className="text-sm text-text-secondary mb-2">
            Powered by The Curious Teens
          </p>
